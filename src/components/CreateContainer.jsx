...
const CreateContainer = () => {
  ...
  const uploadImage = (e) => {
    ...
    const storageRef = ref(storage, `Images/${Date.now()}-${imageFile.name}`); // disimpen ke database, dalam bentuk string
    const uploadTask = uploadBytesResumable(storageRef, imageFile); // disimpan file gambarnya
  ...
  const deleteImage = () => {
    ...
    deleteObject(deleteRef).then(() => {
      ...
      setIsLoading(false);
      setFields(true);
      setMsg("Image deleated successfully");
      setAlertStatus("success");
      setTimeout(() => {
        setFields(false);
      }, 4000);
  ...
  const saveDetails = () => {
    try {
      if (!title || !calories || !imageAsset || !price || !category) {
        setFields(true);
        setMsg("Required fields can's be empty");
        setAlertStatus("danger");
        setTimeout(() => {
          setFields(false);
          setIsLoading(false);
  ...
  return (
    <div className="w-full min-h-screen flex items-center justify-center">
      <div
        className="w-[90%] md:w-[75%] border p-4 border-gray-300 rounded-lg
